...
// تخزين البيانات على GitHub Gist
async function saveToCloud(data) {
    try {
        // خلي المستخدم يدخل GitHub Token
        const token = localStorage.getItem('github_token');
        
        if (!token) {
            // إذا مش عامل تسجيل، استخدم LocalStorage بس
            localStorage.setItem('mizanati_backup', JSON.stringify(data));
            return false;
        }
        
        // استخدم GitHub Gist API
        const response = await fetch('https://api.github.com/gists', {
            ...
            body: JSON.stringify({
                description: 'Mizanati Data Backup',
                public: false,
                files: {
                    'mizanati.json': {
        ...
        console.log('Cloud save failed, saving locally');
        localStorage.setItem('mizanati_backup', JSON.stringify(data));
    }
    return false;
}
...
async function loadFromCloud() {
    ...
    try {
        ...
        const result = await response.json();
        return JSON.parse(result.files['mizanati.json'].content);
    } catch (error) {
        return null;
    }
}
...
// عدّل دالة saveTransaction لتشمل السحابة
async function saveTransaction(e) {
    e.preventDefault();
    
    // ... كود حفظ المعاملة القديم ...
    
    // بعد الحفظ، خزن في السحابة
    const allData = {
    ...
    await saveToCloud(allData);
}

// عدّل init لتجلب من السحابة
async function init() {
    // حاول تجيب من السحابة أولاً
    const cloudData = await loadFromCloud();
    if (cloudData) {
        app.transactions = cloudData.transactions || [];
        app.debts = cloudData.debts || [];
